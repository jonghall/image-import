...
runcmd:
 ...
 - sudo echo "export=REDIS_CERTFILE=$(ibmcloud cdb cxn ${redisinstance} --endpoint-type private --json | jq -r '.[].cli.environment.REDIS_CERTFILE')" >> /root/.bashrc
 - sudo rediss=$(ibmcloud cdb cxn ${redisinstance} --endpoint-type private --json | jq '.[].rediss')
 - sudu echo "export REDISURL=$(echo $rediss | jq -r '.hosts[0].hostname'):$(echo $rediss | jq -r '.hosts[0].port')$( echo $rediss | jq -r '.path')" >> /root/.bashrc
