...
dd if=/dev/vde conv=sync,noerror bs=64K of=/mnt/image-working/image.img status=progress
...
qemu-img convert -p -f raw -O qcow2 /mnt/image-working/image.img /mnt/cos/$snapshotname.qcow2

# delete RAW image and detach volume
echo "Deleting temporary RAW image file."
rm -f /mnt/image-working/image.img
